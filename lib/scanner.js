...
export async function createScanner({
  ...
  const eventAggregates = {
    ...
    values: [], // Track all values for median calculation
  };
          ...
          if (eventName === "Unknown") {
            ...
            if (log.data) {
              try {
                ...
                eventAggregates.values.push(value);
                ...
                // Calculate median
                const sortedValues = [...eventAggregates.values].sort(
                  (a, b) => {
                    if (a < b) return -1;
                    if (a > b) return 1;
                    return 0;
                ...
                eventAggregates.median =
                  sortedValues.length % 2 === 0
                    ? (sortedValues[mid - 1] + sortedValues[mid]) / BigInt(2)
                    : sortedValues[mid];
