...
export async function createScanner({
  ...
  let query = {
    ...
    fieldSelection: {
      log: [LogField.Topic0, LogField.Data],
    },
  ...
  // This handles both '0x' prefixed and non-prefixed hex strings
  const hexToBigInt = (hex) => {
    try {
      ...
      console.error(`Error converting hex to BigInt: ${hex}`, error);
      return BigInt(0);
    }
  };

  // Function to format BigInt with commas
  ...
  const updateDataAggregationDisplay = (eventAggregates) => {
    try {
      ...
      const hasEvents = eventAggregates.Total > 0;

      // Get all event names (excluding Unknown, Total, and sum)
      const eventNames = Object.keys(eventAggregates).filter(
        (key) => key !== "Unknown" && key !== "Total" && key !== "sum" && key !== "min" && key !== "max" && key !== "average"
      );

      // Calculate the total (excluding unknown, total, and sum)
      const knownTotal = Math.max(
        eventNames.reduce((sum, name) => sum + eventAggregates[name], 0),
        1 // Ensure we don't divide by zero
      );

      // Find max count for scaling
      const maxCount = Math.max(
        ...eventNames.map((name) => eventAggregates[name]),
        1
      );
      ...
      const content = eventNames
        .map((name, index) => {
          ...
          return `${neonCyanText("Count".padEnd(labelWidth))} ${eventAggregates[name]}`;
        })
        .join("\n") + "\n" + 
  ...
  try {
    ...
    while (true) {
      ...
      if (
        ...
        res.data.logs.forEach((log) => {
          ...
          if (eventName === "Unknown") {
            ...
            if (log.data) {
              try {
                // Convert hex data to BigInt and add to sum
                ...
                // Update average
                eventAggregates.average = eventAggregates.sum / BigInt(eventAggregates.Total);
              } catch (error) {
        ...
        try {
          if (eventAggregates.Total % 1000 === 0 && res.data.logs[0]) {
      ...
      const eventsPerSecond = (eventAggregates.Total / seconds).toFixed(1);
    ...
    stats.setContent(
      `${neonCyanText("Blocks Scanned")}: ${formatNumber(height)}\n` +
        ...
        `${neonCyanText("Avg Speed")}: ${formatNumber(
          Math.round(eventAggregates.Total / totalTime)
        )} events/s`
    );
